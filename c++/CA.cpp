...
int CA (struct Feature_Info *FeatVect, int NumOfVectors, int Dim, int NumClust , double **Center, int MaximumIt, double Eita_0, double *MinPts, double *MinPts2, double TAU, int EXPINC, int EXPDEC, double m_p, double **W, char *ca_path)
{
	...
	void EucDistance (struct Feature_Info *,double **,int,int,int,double, double **);
	...
	// pjd
	// new stuff for printing along the way
	time_t timing = time(NULL);
	std::ofstream myfile;
	char log_file[1024];
	
	// '/private/tmp/TUF_pjd/super_voxel_model/ca/';
	snprintf( log_file, 1023, "%s/log/foo_%ld.txt", ca_path, timing );
	printf( "%s\n", log_file );
	myfile.open( log_file );

	myfile << std::endl;
	myfile << "Eta:  " << Eita_0 << std::endl;
	myfile << "Tau:  " << TAU << std::endl << std::endl;
    ...
    while (IterNum < MaximumIt){
		...
		EucDistance (FeatVect,Center,NumClust,NumOfVectors,Dim, m_p, W);
		...
		for (i=0, CenDiff=0.0; i<NumClust; i++)
			for (j=0; j<Dim; j++) CenDiff += std::abs(PreviousCenter[i][j]-Center[i][j]);

		// pjd
		myfile << "IterNum:  " << IterNum << std::endl;
		myfile << "NumClus:  " << NumClust << std::endl;
		myfile << "AggCte :  " << AggCte << std::endl;
		myfile << "CenDiff:  " << CenDiff;
		myfile << " < " << MAX_CENT_DIFF << "  :MAX_CENT_DIFF";
	...
	} // closes while ( IterNum < MaximumIt ) {
...
/**************************************************************************************************/
void EucDistance (struct Feature_Info *FeatVector, double **Center, int NumClust, int NumOfVectors, int Dim, double m_p, double **W)
{
	int i, j, k; 
	double temp, temp1;
	double *v, *vW;
	...
	// adjust value assigned to temp to be mahalanobis along
	// each feature vector axis
	// calculate scale factor from the origin
	// pass that in and use here ( divide by )

	for ( i = 0 ; i < NumOfVectors; i++ ){
		for ( j = 0 ; j < NumClust; j++ ){ 
			for ( k = 0; k < Dim; k++ ){
				v[k] = ( (FeatVector+i)->dimen[k]-Center[j][k] );
			}

			// need to perform ( v * W ) * v'				
			// if v is 1xDim
			//    W is Dim x Dim
			//
			// then
			//    ( ( 1 x Dim ) * ( Dim x Dim ) ) * ( Dim x 1 )
			//    ( ( 1 x Dim ) * ( Dim x 1 ) )
			//    ( 1 x 1 )

			VecMatMult ( v, W, vW, Dim, Dim, Dim );

			// temp is vWv'
			temp = DotProduct ( vW, v, Dim );
...
void AssignPts (struct Feature_Info *FeatVector, int NumClust, int NumOfVectors)
{
	...
	for (i=0; i<NumOfVectors; i++){
		for (j=0, min=INFINITY_1; j<NumClust; j++){
